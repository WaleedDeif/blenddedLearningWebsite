...
<section class="sub-page-banner dna-cover dna-cover-4 text-center" data-stellar-background-ratio="0.6">
    ...
    <div class="container">
        <h1 class="entry-title">عرض اجاباتى فى <?php echo $exam_title;?></h1>
    </div>
...
if($question_num>0)
{
	...
	while($row=mysqli_fetch_assoc($res_ques))
	{
		...
		$default_answer = null;
		$sq1 = "SELECT `question_id` , `answer_id` , `student_id` FROM `result` WHERE `question_id` = '{$question_id}' AND `student_id` = '{$student_id}' LIMIT 1";
		$qsq1 = mysqli_query($GLOBALS['connection'],$sq1);
		if($qsq1){
			if(mysqli_num_rows($qsq1) > 0){
				$sanss = mysqli_fetch_assoc($qsq1);
				$default_answer = $sanss['answer_id'];
			}
		}
		$img_link=null;
